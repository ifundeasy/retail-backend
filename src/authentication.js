...
module.exports = function (Glob, locals, compile) {
    return async function (req, res, next) {
        ...
        try {
            //checking data
            let session = await compile(`SELECT * FROM personSession WHERE value = ? AND op_id IN (1, 2)`, token);
            ...
            if (path === '/logout') {
                let updateSession = await compile(`UPDATE personSession SET op_id = 3 WHERE id = ?`, session.id);
            ...
            //lookup user
            let user = await compile(`SELECT * FROM person WHERE id = ? AND op_id IN (1, 2)`, session['person_id']);
            ...
            if (resave) {
                ...
                let updateSession = await compile(`UPDATE person_session SET op_id = 3 WHERE id = ?`, session.id);
